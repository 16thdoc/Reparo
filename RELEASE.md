...
4. Update `deploy\reparo-release.json` to the reviewed release version, immutable
   commit URL, and SHA-256. `Ninja-Reparo-GitHub.ps1` reads this release channel, so
   existing Ninja deployments need no script re-import. Update `Ninja-GitHub.ps1` only
   when its direct-use default source pin is intentionally advanced.
