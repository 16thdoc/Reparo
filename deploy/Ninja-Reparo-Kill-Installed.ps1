<#
.SYNOPSIS
Runs Reparo -Kill from an existing runtime and refreshes the Ninja version field.
...
with -Kill, then updates Ninja's Reparo device text custom field even if the kill
pass fails.
#>
[CmdletBinding()]
param()
...
Write-Host '=== Ninja Reparo Kill (Installed Runtime) ==='
...
Write-Host 'Running installed Reparo with -Kill.'
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $runtimePath -Kill -LogRoot $logRoot
$reparoExitCode = $LASTEXITCODE

Update-NinjaReparoCustomField -ReparoPath $runtimePath
if ($reparoExitCode -ne 0) {
    throw "Reparo -Kill failed with exit code $reparoExitCode. The Ninja Reparo version field was still refreshed."
}

Write-Host 'Installed Reparo -Kill completed successfully.'
