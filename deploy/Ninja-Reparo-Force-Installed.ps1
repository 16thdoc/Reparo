<#
.SYNOPSIS
Runs Reparo -Force from an existing runtime and refreshes the Ninja version field.
...
with -Force, then updates Ninja's Reparo device text custom field even if the
maintenance pass fails.
#>
[CmdletBinding()]
param()
...
Write-Host '=== Ninja Reparo Force (Installed Runtime) ==='
...
Write-Host 'Running installed Reparo with -Force.'
...
Write-Host 'Installed Reparo -Force completed successfully.'
