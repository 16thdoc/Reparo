<#
.SYNOPSIS
Runs Reparo -Force -AllowReboot from an existing runtime and refreshes the Ninja version field.
...
with -Force -AllowReboot, then updates Ninja's Reparo device text custom field even if
the maintenance pass fails. Windows Update may reboot only when it requires one.
#>
[CmdletBinding()]
param()
...
Write-Host '=== Ninja Reparo Force + AllowReboot (Installed Runtime) ==='
...
Write-Host 'Running installed Reparo with -Force -AllowReboot.'
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $runtimePath -Force -AllowReboot -LogRoot $logRoot
$reparoExitCode = $LASTEXITCODE

Update-NinjaReparoCustomField -ReparoPath $runtimePath
if ($reparoExitCode -ne 0) {
    throw "Reparo -Force -AllowReboot failed with exit code $reparoExitCode. The Ninja Reparo version field was still refreshed."
}

Write-Host 'Installed Reparo -Force -AllowReboot completed successfully.'
